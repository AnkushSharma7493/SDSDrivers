...
public class FireStoreDutyService {
    ...
    public void getDutiesByDriverPhone(String phone, FireStoreQueryDutyCallback callback){

        if(ApplicationContext.isSkipCache()){
            getDutiesByDriverPhoneStatusFireStore(phone, callback);
        } else {
            db.collection(COLLECTION_NAME)
                    .orderBy(ETAG_FIELD, Query.Direction.DESCENDING)
                    .limit(1)
    ...
    // Query by status
    private void getDutiesByDriverPhoneStatusFireStore(String phone, FireStoreQueryDutyCallback callback) {
            db.collection(COLLECTION_NAME)
                    .whereEqualTo("driverPhone", phone)
                    .orderBy("dutyReportingDate", Query.Direction.DESCENDING)
                    .get()
                    .addOnSuccessListener(queryDocumentSnapshots -> {
                        List<Duty> results = new ArrayList<>();
                        for (DocumentSnapshot doc : queryDocumentSnapshots) {
...
//    public ListenerRegistration getDutiesByDriverPhoneNStatusFireStore(String phone, DutyStatus status, FireStoreQueryDutyCallback callback) {
